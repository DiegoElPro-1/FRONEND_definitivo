...
export default function CatEstadosAliados() {
  ...
  const onGuardar = async (item) => {
    if (item.idEstadoAliado) {
      const data = await actualizarEstadoAliado(item.idEstadoAliado, { nombre: item.nombre, descripcion: item.descripcion });
      setDatos(prev => prev.map(d => d.idEstadoAliado === item.idEstadoAliado ? data.estado : d))
    } else {
      const data = await crearEstadoAliado({ nombre: item.nombre, descripcion: item.descripcion });
