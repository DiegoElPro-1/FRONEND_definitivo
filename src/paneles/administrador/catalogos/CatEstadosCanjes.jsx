...
export default function CatEstadosCanjes({ showToast }) {
  ...
  const onGuardar = async (item) => {
    if (item.idEstadoCanje) {
      const data = await actualizarEstadoCanje(item.idEstadoCanje, { nombre: item.nombre, descripcion: item.descripcion });
      setDatos(prev => prev.map(d => d.idEstadoCanje === item.idEstadoCanje ? data.estado : d))
    } else {
      const data = await crearEstadoCanje({ nombre: item.nombre, descripcion: item.descripcion });
